...
function renderAIAnalytics(aiData) {
    ...
    const trendColor = trend === 'increasing' ? 'var(--teal)' : (trend === 'decreasing' ? 'var(--red)' : 'var(--text-dim)');
...
// ============================================================
// دالة تحديث التحليلات الذكية بشكل دوري
// ============================================================
let aiAnalyticsInterval = null;

function startAIAnalyticsUpdater() {
    if (aiAnalyticsInterval) clearInterval(aiAnalyticsInterval);
    aiAnalyticsInterval = setInterval(async () => {
        if (business && document.getElementById('view-dashboard').classList.contains('active')) {
            try {
                const { start, end } = getAnalyticsRange();
