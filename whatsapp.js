...
let whatsappConfig = {
    ...
    provider: 'callmebot',
    webhookUrl: '',
    // ✅ إضافة خيارات التنبيهات التلقائية
    autoReports: {
        shiftClosed: true,      // إقفال الشيفت
        dailyReport: true,      // تقرير اليوم
        weeklySummary: true,    // ملخص الأسبوع
        forecast: true,         // توقعات الأسبوع
        analytics: false        // تحليلات (افتراضي غير مفعل)
    }
};
...
function loadWhatsappConfig() {
    try {
        ...
        if (saved) {
            ...
            // ✅ دمج الإعدادات القديمة مع الإعدادات الجديدة
            whatsappConfig = {
                ...whatsappConfig,
                ...parsed,
                autoReports: {
                    ...whatsappConfig.autoReports,
                    ...(parsed.autoReports || {})
                }
            };
...
// ✅ تحديث إعدادات التنبيهات التلقائية
function updateAutoReportSetting(key, value) {
    if (whatsappConfig.autoReports.hasOwnProperty(key)) {
        whatsappConfig.autoReports[key] = value;
        saveWhatsappConfig();
        renderWhatsappSettingsUI();
        showToast(t('تم تحديث إعدادات التنبيهات', 'Notification settings updated'), 'success');
    }
}
...
// ============================================================
// التنبيهات التلقائية (مع التحقق من الإعدادات)
// ============================================================
...
function startWhatsappAutoReports() {
    ...
    whatsappAutoReportInterval = setInterval(() => {
        ...
        if (hour === 23 && minute >= 59 && minute <= 59) {
            if (whatsappConfig.autoReports?.dailyReport !== false) {
                console.log('📱 Sending daily auto report...');
                sendDailyReport();
            }
            
            // ✅ توقعات الأسبوع مع التقرير اليومي (إذا كانت مفعلة)
            if (whatsappConfig.autoReports?.forecast !== false) {
                setTimeout(() => {
                    console.log('📱 Sending forecast auto report...');
                    sendForecastReport();
                }, 5000);
            }
        }
        ...
        if (day === 0 && hour === 10 && minute <= 1) {
            if (whatsappConfig.autoReports?.weeklySummary !== false) {
                console.log('📱 Sending weekly summary auto report...');
                sendWeeklySummary();
            }
            
            // ✅ تحليلات أسبوعية (إذا كانت مفعلة)
            if (whatsappConfig.autoReports?.analytics === true) {
                setTimeout(() => {
                    console.log('📱 Sending analytics auto report...');
                    sendAnalyticsReport('week');
                }, 3000);
            }
        }
    }, 60000);
...
// ============================================================
// واجهة إعدادات واتساب (مع خيارات التنبيهات)
// ============================================================
...
function renderWhatsappSettingsUI() {
    ...
    const autoReports = whatsappConfig.autoReports || {};

    // حساب عدد التنبيهات المفعلة
    const enabledCount = Object.values(autoReports).filter(v => v === true).length;
    const totalCount = Object.keys(autoReports).length;
    ...
    container.innerHTML = `
        <div class="panel" style="padding:16px; margin-bottom:12px;">
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px; padding-bottom:12px; border-bottom:1px solid var(--border);">
                <div>
                    <div style="font-weight:700; font-size:16px;">📱 ${t('تنبيهات واتساب', 'WhatsApp Notifications')}</div>
            ...
            <div id="whatsappSettingsForm" style="${enabled ? 'display:block' : 'display:none'}">
                ...
                <div style="background:var(--bg-sunken); border-radius:var(--radius-sm); padding:12px; margin-bottom:14px;">
                    <div style="font-weight:600; font-size:13px; color:var(--text-dim); margin-bottom:8px;">${t('الإعدادات الأساسية', 'Basic Settings')}</div>
                    
                    <div class="field" style="margin-bottom:8px;">
                        <label style="font-size:11px; color:var(--text-dim);">${t('رقم الهاتف (بصيغة دولية)', 'Phone Number (International format)')}</label>
                        <input type="text" id="whatsappPhone" class="mono" value="${phone}" placeholder="مثال: 201234567890" style="direction:ltr;text-align:left; font-size:13px; padding:8px 12px; width:100%;">
                        <div style="font-size:9px; color:var(--text-faint); margin-top:3px;">${t('أدخل الرقم بدون + أو 00، مثال: 201234567890', 'Enter number without + or 00, e.g., 201234567890')}</div>
                    </div>

                    <div class="field" style="margin-bottom:8px;">
                        <label style="font-size:11px; color:var(--text-dim);">${t('مفتاح API', 'API Key')}</label>
                        <input type="text" id="whatsappApiKey" class="mono" value="${apiKey}" placeholder="مفتاح API من CallMeBot" style="font-size:13px; padding:8px 12px; width:100%;">
                        <div style="font-size:9px; color:var(--text-faint); margin-top:3px;">
                    ...
                    <div class="field" style="margin-bottom:0;">
                        <label style="font-size:11px; color:var(--text-dim);">${t('مزود الخدمة', 'Service Provider')}</label>
                        <select id="whatsappProvider" style="font-size:13px; padding:8px 12px; width:100%; background:var(--bg); border:1px solid var(--border); border-radius:var(--radius-sm); color:var(--text);">
                ...
                <div style="background:var(--bg-sunken); border-radius:var(--radius-sm); padding:12px; margin-bottom:14px;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
                        <div style="font-weight:600; font-size:13px; color:var(--text-dim);">${t('التنبيهات التلقائية', 'Auto Notifications')}</div>
                        <div style="font-size:10px; color:var(--text-faint);">${t('اختر ما تريد استلامه تلقائياً', 'Choose what you want to receive automatically')}</div>
                    </div>
                    
                    <!-- 🔒 إقفال الشيفت -->
                    <div class="whatsapp-option">
                        <div class="option-info">
                            <div class="option-title">🔒 ${t('إقفال الشيفت', 'Shift Closure')}</div>
                            <div class="option-desc">${t('تقرير كامل عند إقفال الشيفت', 'Full report when shift is closed')}</div>
                        </div>
                        <input type="checkbox" id="autoShiftClosed" ${autoReports.shiftClosed !== false ? 'checked' : ''} 
                               onchange="updateAutoReportSetting('shiftClosed', this.checked)">
                    </div>

                    <!-- 📊 تقرير اليوم -->
                    <div class="whatsapp-option">
                        <div class="option-info">
                            <div class="option-title">📊 ${t('تقرير اليوم', 'Daily Report')}</div>
                            <div class="option-desc">${t('ملخص أداء اليوم في نهاية اليوم (23:59)', 'Daily performance summary at end of day (23:59)')}</div>
                        </div>
                        <input type="checkbox" id="autoDailyReport" ${autoReports.dailyReport !== false ? 'checked' : ''} 
                               onchange="updateAutoReportSetting('dailyReport', this.checked)">
                    </div>

                    <!-- 📈 توقعات الأسبوع -->
                    <div class="whatsapp-option">
                        <div class="option-info">
                            <div class="option-title">📈 ${t('توقعات الأسبوع', 'Weekly Forecast')}</div>
                            <div class="option-desc">${t('توقعات الإيراد والمصروفات للأسبوع القادم', 'Revenue and expense forecast for next week')}</div>
                        </div>
                        <input type="checkbox" id="autoForecast" ${autoReports.forecast !== false ? 'checked' : ''} 
                               onchange="updateAutoReportSetting('forecast', this.checked)">
                    </div>

                    <!-- 📆 ملخص الأسبوع -->
                    <div class="whatsapp-option">
                        <div class="option-info">
                            <div class="option-title">📆 ${t('ملخص الأسبوع', 'Weekly Summary')}</div>
                            <div class="option-desc">${t('تقرير شامل عن أداء الأسبوع الماضي (الأحد 10:00)', 'Comprehensive report on last week\'s performance (Sunday 10:00)')}</div>
                        </div>
                        <input type="checkbox" id="autoWeeklySummary" ${autoReports.weeklySummary !== false ? 'checked' : ''} 
                               onchange="updateAutoReportSetting('weeklySummary', this.checked)">
                    </div>

                    <!-- 📊 تحليلات -->
                    <div class="whatsapp-option" style="border-bottom:none;">
                        <div class="option-info">
                            <div class="option-title">📊 ${t('تحليلات متقدمة', 'Advanced Analytics')}</div>
                            <div class="option-desc">${t('تقرير تحليلات مفصل (أسبوعي - الأحد)', 'Detailed analytics report (weekly - Sunday)')}</div>
                        </div>
                        <input type="checkbox" id="autoAnalytics" ${autoReports.analytics === true ? 'checked' : ''} 
                               onchange="updateAutoReportSetting('analytics', this.checked)">
                    </div>
                </div>

                <!-- ✅ حالة التنبيهات التلقائية -->
                <div class="whatsapp-status-summary">
                    <span>${t('التنبيهات التلقائية:', 'Auto notifications:')}</span>
                    <span>
                        <span class="count">${enabledCount}</span> ${t('مفعلة من', 'enabled of')} <span class="count">${totalCount}</span>
                        ${enabledCount === 0 ? ' <span class="warning">⚠️ ' + t('جميع التنبيهات معطلة', 'All notifications are disabled') + '</span>' : ''}
                    </span>
                </div>

                <!-- ✅ أزرار الحفظ والإرسال اليدوي -->
                <button class="btn btn-teal btn-block" onclick="saveWhatsappSettingsFromUI()" style="margin-top:12px; margin-bottom:10px;">
                    <i class="fa-solid fa-floppy-disk"></i> ${t('حفظ الإعدادات', 'Save Settings')}
                </button>

                <div style="font-size:10px; color:var(--text-faint); text-align:center; margin-bottom:10px;">
                    ⚡ ${t('الرسائل ترسل في الخلفية ولا تنتظر الرد', 'Messages are sent in the background without waiting for a reply')}
                </div>

                <div style="display:flex; gap:6px; flex-wrap:wrap;">
                    <button class="btn btn-amber btn-sm" onclick="testWhatsappMessage()" style="flex:1; min-width:55px; font-size:10px; padding:5px 8px;">
                        <i class="fa-solid fa-paper-plane"></i> ${t('اختبار', 'Test')}
                    </button>
                    <button class="btn btn-amber btn-sm" onclick="sendDailyReport()" style="flex:1; min-width:55px; font-size:10px; padding:5px 8px;">
                        <i class="fa-solid fa-calendar-day"></i> ${t('تقرير اليوم', 'Daily')}
                    </button>
                    <button class="btn btn-amber btn-sm" onclick="sendForecastReport()" style="flex:1; min-width:55px; font-size:10px; padding:5px 8px;">
                        <i class="fa-solid fa-chart-line"></i> ${t('التوقعات', 'Forecast')}
                    </button>
                </div>
                <div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:4px;">
                    <button class="btn btn-amber btn-sm" onclick="sendWeeklySummary()" style="flex:1; min-width:55px; font-size:10px; padding:5px 8px;">
                        <i class="fa-solid fa-calendar-week"></i> ${t('ملخص الأسبوع', 'Weekly')}
                    </button>
                    <button class="btn btn-amber btn-sm" onclick="sendAnalyticsReport('week')" style="flex:1; min-width:55px; font-size:10px; padding:5px 8px;">
                        <i class="fa-solid fa-chart-pie"></i> ${t('تحليلات', 'Analytics')}
                    </button>
                    <button class="btn btn-amber btn-sm" onclick="sendShiftClosedAlert(currentShift, getShiftTotals(currentShift))" style="flex:1; min-width:55px; font-size:10px; padding:5px 8px;">
...
window.updateAutoReportSetting = updateAutoReportSetting;
